        ...
        <Menu as="div" class="relative inline-block text-left">
            <div>
                <MenuButton
                    ...
                    Cuisines
                    <ph-caret-down :size="20" class="-mr-1 ml-2 h-5 w-5 text-violet-200 hover:text-violet-100"
                        aria-hidden="true" />
                </MenuButton>
            </div>
                ...
                <MenuItems
                    ...
                    <div class="px-1 py-1">
                        <template :key="strISO" v-for="{ strISO, strArea } in strAreaList.slice(0, 10)">
                            ...
                            <RouterLink :to="`/category/a=${strArea}`.toLowerCase()" :class="[
                                active ? 'bg-color-accent text-white' : 'text-gray-900',
                                'group flex w-full items-center rounded-md px-2 py-2 text-md font-body',
                            ]">
                                <img :src="`https://flagcdn.com/w20/${strISO.toLowerCase()}.webp`"
                                    :alt="`flag of ${strArea} `" srcset="" class="mr-2">
                                {{ strArea }}
                            </RouterLink>
                            </MenuItem>
                        </template>
