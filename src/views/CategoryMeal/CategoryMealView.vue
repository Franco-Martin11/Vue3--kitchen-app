...
<template>
    <LayoutContainer>
        <template #content>
            <div v-motion-fade class="flex flex-wrap md:gap-4 justify-center flex-1 px-4 my-8 md:my-2 h-full">
                <h2> Category Meal
                </h2>
...
const fetchData = async (id: string | number | string[]) => {
    try {
        const response: AxiosResponse<FilterMealByCategoryResponse> = await $axios.get(apiUrls.filterByCategory(), { params: { c: id } })
